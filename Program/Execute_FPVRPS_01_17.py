...
import FPVRP_PostProcessing_NEW as postprocessor
from FPVRPS_RouteVizualizer import PVRP_Vizualizer
from FPVRP_Inputs import InputGISReader, Scenario, DummyForExcelInterface
from SETUP_and_IO_NEW import IOExcel
...
def optimize_scenario(scenario, framework_input, io_excel):
    ...
    model.set_objective()
    model.solve_model()
    io_excel.save_gurobi_res_in_excel([model.z, model.y, model.q], model.mp.objVal)
...
def postprocess_grb_results( scenario, framework_input, io_excel_grb_results, io_excel_processed_results):
    ...
    io_excel_processed_results.save_df_res_in_excel([df_grb_res_adjusted['Z'], df_grb_res_adjusted['Y'], df_grb_res_adjusted['Q']],
                                  new_total_costs, "DecisionvariableValues_PP.xlsx")
...
scenario = Scenario(9, lower_bound=50, upper_bound=80, GIS_inputs=input_gis)
...
io_excel = IOExcel(scenario, root_directory='01-27-Results_FP-VRPs', add_to_folder_title='_Test', title_excel_to_create_or_read="DecisionvariableValues.xlsx")
optimize_scenario(scenario, framework_input, io_excel)

# io_excel_for_processed_data = IOExcel(scenario, root_directory='01-18-Results_FP-VRPs', add_to_folder_title='_Cov33_Test', title_excel_to_create_or_read="DecisionvariableValues_PP.xlsx")
# postprocess_grb_results( scenario, framework_input, io_excel, io_excel_for_processed_data)
#
# vizualize_results(scenario, framework_input, io_excel, '01-18-Results_FP-VRPs')
