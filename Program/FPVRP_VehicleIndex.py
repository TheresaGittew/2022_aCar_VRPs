...
from DEPR_ReadNilsInputFiles import read_odmatrix
...
class FPVRPVehInd:
    ...
    def __initialize_variables(self):
       ...
       # self.l = self.mp.addVars(self.cfg.A, self.K, self.cfg.T, vtype=GRB.CONTINUOUS, name='l') # loads
        self.q = self.mp.addVars(self.C, self.K, self.cfg.T, vtype=GRB.CONTINUOUS, name='q') # deliveries to users
    ...
    def __set_default_constraints(self):
        ...
        self.mp.addConstrs(self.q[i, k, t] <= self.cfg.w[i]['PNC'] * self.z[i, k, t] for i in self.C for k in self.K for t in self.cfg.T)
        ...
        self.mp.addConstrs(quicksum(self.q[i, k, t] for i in self.C) <= self.cfg.Q * self.z[0, k, t] for k in self.K for t in self.cfg.T)
        ...
        self.mp.addConstrs(quicksum(self.y[i, j, k, t] for j in self.N if (i, j) in self.cfg.A) == self.z[i,k,t] for i in self.N for k in self.K for t in self.cfg.T)
        ...
        self.mp.addConstrs(quicksum(self.q[i,k,t] for t in self.cfg.T for k in self.K) >= self.cfg.W[i]['PNC'] for i in self.C)
    ...
    def solve_model(self):
        self.mp.Params.MIPGap = 0.05  # self.mp.Params.TimeLimit = 5000
...
class FPVRPVecIndConfg:
    ...
    def __init__(self, T, A, W_i, w_i, capa, c, coordinates, S, subset_ids, subset_id_to_C, subset_id_to_A):
