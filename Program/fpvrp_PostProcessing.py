...
import numpy as np
from Program.Depr_Results.k_opt_heuristics import tsp_2_opt

index_hub = 100

# #
# Method that are useful for checking routes and, possibly, improving them

# #
# Class designed to get gurobi results and checks for improvement potential
# Very easy: All Arcs in every computed route are exchanged and then,
# we check if the resulting route is better than the old one.
# Sometimes, tiny enhancements are possible.
...
class FPVRPPostProcessor():
    ...
    # Aux method to identify faulty routes
    ...
    # aux method to detect cost differences between old and new route
...
# #
# Route Validation Class
# Used in fvrp_GRBModel_XL.py for checking if invalid subtours are part of the solution
# Class gets a list of arcs and checks if valid or not.
# Further some more probably useful methods have been implemented that "repair" faulty routes
...
class RouteValidation():
    ...
    def correct_faulty_routes(self):
        ...
        built_arcs[-1] = built_arcs[-1][0], faulty_arc_list_rest[0][
            0]  # replace last destination in built list with first origin in remaining list
    ...
    # checks if these links indicate that there are subtours within the route and returns false in this case,
    # otherwise true

    def __check_if_route_has_no_subtours(self, arc_list):
        starter_elem_arc_list = next(filter(lambda x: x[0] == index_hub, arc_list),
                                     None)  # we start with arc that leaves hub
