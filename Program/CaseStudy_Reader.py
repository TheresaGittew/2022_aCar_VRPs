...
# #
# Auxiliary class for extracting the obtained solutions (excel files) for single scenarios
# Relevant key data (for example, obj. value) is summarized in output file
# specify folder_name => root directory which includes all computed folders which correspond to a single scenario
# each folder should have a name like this: 'scenario_['ED'] 9 0.22', (usually created when using Execution.py)

num_digits_round = 2
class Result_Summarizer():

    def __init__(self, rel_path_to_root_directory_to_read, rel_path_to_root_directory_to_write,
        ...
        self.path_to_output = rel_path_to_root_directory_to_write

        self.folder_name = rel_path_to_root_directory_to_read
...
Result_Summarizer(rel_path_to_root_directory_to_read='/MA_Thesis_Results/CaseStudies/CI_NumericalResults', rel_path_to_root_directory_to_write='/Program',
                  relative_path_to_demand='/GIS_Data/IC_Location_Data.csv',
                  relative_path_to_coors='/GIS_Data/IC_Coordinates.csv',
                  relative_path_to_od_matrix='/GIS_Data/IC_ODs.csv')
